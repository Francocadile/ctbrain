...
export default function ClientUsers() {
  // Evitamos desestructurar por si el hook no está inicializado aún
  const sessionHook = useSession();
  const myEmail = sessionHook?.data?.user?.email ?? "";
    ...
    <div className="space-y-8">
      ...
      <form onSubmit={onCreate} className="space-y-3 rounded-xl border border-white/10 p-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            <label className="mb-1 block text-xs text-white/60">Rol</label>
            <select
              value={role}
          ...
          <div>
            <label className="mb-1 block text-xs text-white/60">Contraseña (mín. 6)</label>
            <input
              type="password"
              minLength={6}
              value={password}
