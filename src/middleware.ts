...
function isClinicalReadForCT(pathname: string, method: string) {
  return method === "GET" && /^\/api\/medico\/clinical(?:\/|$)/.test(pathname);
}

export async function middleware(req: any) {
  // ⬇️ importamos NextResponse aquí para evitar duplicados de identificadores
  ...
  // Rutas públicas (con control de método en /api/users)
  if (matchAny(pathname, PUBLIC)) {
    if (/^\/api\/users(?:\/|$)/.test(pathname) && req.method !== "POST") {
      return new NextResponse(JSON.stringify({ error: "Method Not Allowed" }), {
        status: 405,
  ...
  // MÉDICO (+ excepción lectura para CT)
  if (needsMED) {
    const allowCTReadOnly = role === "CT" && isClinicalReadForCT(pathname, req.method);
    const allowed = role === "MEDICO" || role === "ADMIN" || allowCTReadOnly;
    if (!allowed) {
...
export async function middleware(req: any) {
  const { NextResponse } = await import("next/server"); // ← evita duplicados
  const url = new URL(req.url);
  const pathname = url.pathname;
  const api = isApi(pathname);
  ...
  const token = await getNextAuthToken({ req, secret: process.env.NEXTAUTH_SECRET });
  ...
  if (role !== "ADMIN" && isApproved === false) {
    ...
    const pendUrl = new URL("/pending-approval", url.origin);
    return NextResponse.redirect(pendUrl);
  }
  ...
  if (needsADM) {
    ...
    if (!allowed) {
      ...
      const home = new URL(roleHome(role), url.origin);
...
export async function middleware(req: NS.NextRequest) {
  ...
  if (matchAny(pathname, PUBLIC)) {
    if (/^\/api\/users(?:\/|$)/.test(pathname) && req.method !== "POST") {
      return new NS.NextResponse(JSON.stringify({ error: "Method Not Allowed" }), {
        status: 405,
        headers: { "content-type": "application/json" },
      });
    }
    return NS.NextResponse.next();
  }
  ...
  if (!token) {
    if (api) {
      return new NS.NextResponse(JSON.stringify({ error: "Unauthorized" }), {
        status: 401,
        headers: { "content-type": "application/json" },
      });
    }
    ...
    return NS.NextResponse.redirect(url);
  }
  ...
  if (role !== "ADMIN" && isApproved === false) {
    if (api) {
      return new NS.NextResponse(JSON.stringify({ error: "Forbidden", pendingApproval: true }), {
        status: 403,
        headers: { "content-type": "application/json" },
      });
    }
  ...
  if (needsADM) {
    ...
    if (!allowed) {
      if (api) {
        return new NS.NextResponse(JSON.stringify({ error: "Forbidden" }), {
          status: 403,
          headers: { "content-type": "application/json" },
        });
      }
