...
function parseTurnFromDesc(description?: string | null): TurnKey | undefined {
  const t = (description || "").trim();
  const m = t.match(/^\[DAYNAME:(morning|afternoon)\]/i);
...
function isDayNameEntry(s: Session) {
  return typeof s.description === "string" && /^\[DAYNAME:(morning|afternoon)\]/i.test(s.description);
}
...
export default function CTSessionsPage() {
  ...
  // Solo nombres de sesión
  const named = useMemo(() => sessions.filter(isDayNameEntry), [sessions]);

  // Orden desc por fecha
  const sorted = useMemo(
    () => [...named].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()),
    [named]
  );
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Listado por nombre de sesión (editor semanal)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border p-6 text-sm text-gray-600">
          No hay nombres de sesión cargados. <br />
          <span className="text-gray-500">
            Cargá el <strong>Nombre de sesión</strong> en <a className="underline" href="/ct/plan-semanal">Plan semanal</a> (fila “Nombre sesión”).
          </span>
        </div>
      ) : (
        <ul className="space-y-3">
          {visible.map((s) => {
            const turn = parseTurnFromDesc(s.description) ?? inferTurnFromISO(s.date);
