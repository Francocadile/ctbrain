...
/** tags */
function parseTurnAndRow(description?: string | null): { turn?: TurnKey; row?: string } {
  const text = (description || "").trim();
  if (!text) return {};
  let m = text.match(/^\[(GRID|DAYFLAG|DAYNAME):(morning|afternoon)(?::(.+?))?\]/i);
  if (m) {
    return { turn: m[2] as TurnKey, row: (m[3] || "").trim() || undefined };
  }
  return {};
}
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setDateFilter("")} className="text-sm px-3 py-1.5 rounded-lg border hover:bg-gray-50"
                  disabled={!dateFilter} title="Quitar filtro">Limpiar</button>
          <button onClick={openCreate} className="px-4 py-2 rounded-xl shadow border text-sm font-medium hover:shadow-md">
            Nueva sesión
          </button>
        </div>
      </header>
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {visible.map((s) => {
            const { turn: parsedTurn } = parseTurnAndRow(s.description);
            const turn = parsedTurn ?? inferTurnFromISO(s.date);

            // Mostrar EXACTAMENTE el nombre guardado (DAYNAME)
            const displayTitle = (s.title || "").trim() || "Sesión sin nombre";
              ...
              <li key={s.id} className="rounded-xl border p-3 shadow-sm flex items-start justify-between bg-white">
                ...
                <div className="flex flex-col items-end gap-2">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(s)} className="text-xs px-3 py-1 rounded-lg border hover:bg-gray-50">Editar</button>
                    <button onClick={() => handleDelete(s.id)} className="text-xs px-3 py-1 rounded-lg border hover:bg-gray-50">Eliminar</button>
                  </div>
                </div>
              </li>
            );
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-5 shadow-xl border">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-lg font-semibold">{editingId ? "Editar sesión" : "Nueva sesión"}</h2>
              <button onClick={() => { setOpen(false); resetForm(); }}
                      className="text-sm px-3 py-1 rounded-lg border hover:bg-gray-50">Cerrar</button>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-[11px] text-gray-500 mt-1">Este título se verá tal cual en la lista de Sesiones.</p>
                </div>
              ...
              <div className="space-y-2">
                ...
                <input value={playerSearch} onChange={(e) => setPlayerSearch(e.target.value)}
                       placeholder="Buscar por nombre o email…" className="w-full rounded-lg border px-3 py-2 text-sm mb-2" />
            ...
            <div className="mt-4 flex items-center justify-end gap-2">
              <button onClick={() => { setOpen(false); resetForm(); }}
                      className="text-sm px-4 py-2 rounded-lg border hover:bg-gray-50">Cancelar</button>
