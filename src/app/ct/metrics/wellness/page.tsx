...
function sumScore(w: Wellness) {
  // suma simple de los 6 ítems (1–5 cada uno) => 6–30
  return w.sleepQuality + w.fatigue + w.soreness + w.stress + w.mood + Math.min(5, Math.max(1, Math.round(w.sleepHours / 2))); 
  // Nota: incluimos horas/2 (cap a 5) como proxy simple para el total (MVP).
}

function colorForScore(deltaPct: number) {
  // Verde  (>= -10%), Amarillo (-10% a -20%), Rojo (< -20%)
...
export default function WellnessCTPage() {
  ...
  const byUser = useMemo(() => {
    ...
    list.forEach(w => {
      if (!m.has(w.userId)) m.set(w.userId, []);
      m.get(w.userId)!.push(w);
    });
    // sort asc por fecha para cálculos de promedio/último
    for (const arr of m.values()) arr.sort((a,b) => a.date.localeCompare(b.date));
    return m;
  }, [list]);

  const rows = useMemo(() => {
    // para cada user => promedio (últimos N), último valor y delta %
    const out = [];
    for (const [uid, arr] of byUser) {
      if (arr.length === 0) continue;
      const last = arr[arr.length - 1];
      const lastScore = sumScore(last);
      const avg = arr.reduce((acc,w)=>acc+sumScore(w),0) / arr.length;
    ...
    // ordenar: rojo primero (delta más negativo)
  ...
  return (
    <div className="p-4 space-y-4">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left p-2">Último total</th>
                <th className="text-left p-2">Promedio</th>
                <th className="text-left p-2">Δ% vs prom</th>
                <th className="text-left p-2">Detalle último</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(r => (
                <tr key={r.userId} className="border-b">
                  ...
                  <td className="p-2">
                    <span className={`px-2 py-0.5 rounded border text-[12px] ${colorForScore(r.deltaPct)}`}>
                      {r.deltaPct}%
                    </span>
                  </td>
                  <td className="p-2">
                    <div className="text-[12px] text-gray-700">
                      Sueño: {r.last.sleepQuality}/5 · {r.last.sleepHours}h · Fatiga: {r.last.fatigue}/5 ·
                      Dolor: {r.last.soreness}/5 · Estrés: {r.last.stress}/5 · Ánimo: {r.last.mood}/5
                      {r.last.notes ? <> · <i>{r.last.notes}</i></> : null}
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {/* Crudo por fecha (para auditar rápido) */}
      <section className="rounded-2xl border bg-white shadow-sm overflow-x-auto">
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b bg-gray-50">
                ...
                <th className="text-left p-2">Fatiga</th>
            ...
            <tbody>
              {list.map(w => (
                <tr key={w.id} className="border-b">
                  ...
                  <td className="p-2">{w.sleepQuality}/5</td>
                  <td className="p-2">{w.sleepHours}</td>
                  <td className="p-2">{w.fatigue}/5</td>
                  <td className="p-2">{w.soreness}/5</td>
                  <td className="p-2">{w.stress}/5</td>
                  <td className="p-2">{w.mood}/5</td>
