...
export default function EditPlayerForm({ player, onClose }: { player: PlayerWithUser; onClose: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/ct/plantel/${player.id}`, {
        method: "PUT",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-md rounded-2xl bg-white p-4 shadow-lg">
        <h2 className="text-sm font-semibold mb-3">Editar jugador</h2>
