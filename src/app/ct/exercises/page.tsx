...
import { searchExercises, type ExerciseDTO } from "@/lib/api/exercises";
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <header className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-500">Todos tus ejercicios, con búsqueda, filtros y orden.</p>
        </div>
      ...
      {loading ? (
        ...
        <ul className="space-y-3">
          {rows.map((ex) => {
            const sessionId = (ex as any).sourceSessionId ?? (ex as any).sessionId ?? null;
            const sessionHref = sessionId ? `/ct/sessions/${sessionId}` : null;
            const exerciseHref = `/ct/exercises/${encodeURIComponent(ex.id)}`;
                ...
                <div className="flex items-center gap-2">
                  {sessionHref ? (
                    <a
                      ...
                      title="Abrir editor de la sesión"
                    >
                      Abrir sesión
                    </a>
                  ) : null}
