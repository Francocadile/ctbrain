...
const DAYNAME_TAG = "DAYNAME";           // description: [DAYNAME:<turn>] | YYYY-MM-DD (título = nombre sesión)
...
function PlanSemanalInner() {
  ...
  function findByTag(dayYmd: string, tag: string, turn: TurnKey): SessionDTO | undefined {
    const list = daysMap[dayYmd] || [];
    return list.find((s) => isTag(s, tag, turn));
  }
  ...
  async function setDayName(dayYmd: string, turn: TurnKey, name: string) {
    ...
    try {
      ...
      // NO recargo la semana acá para no perder foco al tipear; lo hago en blur
    } catch (e: any) {
      console.error(e);
      alert(e?.message || "No se pudo guardar el nombre de la sesión");
    }
  }
      ...
      for (const [k, value] of entries) {
        ...
        if (!existing) {
          ...
          await updateSession(existing.id, {
            title: text,
            description: existing.description?.startsWith(markerStr)
  ...
  /* -------------------- MetaInput -------------------- */
  function MetaInput({ dayYmd, turn, row }: {
      ...
      return (
        <div className="flex items-center justify-between gap-1">
          ...
          <div className="flex items-center gap-1">
            <button type="button" className="h-6 px-1.5 rounded border text-[11px] hover:bg-gray-50"
                    onClick={() => setVideoEditing((m) => ({ ...m, [k]: true }))} title="Editar">✏️</button>
            <button type="button" className="h-6 px-1.5 rounded border text-[11px] hover:bg-gray-50"
                    onClick={() => stageCell(dayYmd, turn, row, "")} title="Borrar">❌</button>
          </div>
        </div>
      );
    }
  ...
  function DayStatusRow({ turn }: { turn: TurnKey }) {
    return (
      <div className="grid items-center border-b bg-gray-50/60"
        ...
        {orderedDays.map((ymd) => {
          const df = getDayFlag(ymd, turn); // siempre leído del estado real
            ...
            <div key={`${ymd}-${turn}-tipo`} className="p-1">
              <div className="flex items-center gap-1">
                <select
                  ...
                  value={df.kind}
                  onChange={(e) => save(e.target.value as DayFlagKind)}
                >
                ...
                {df.kind === "PARTIDO" && (
                  <>
                    <input
                      key={`${ymd}-rv`} // forzar refresco cuando cambia de día/semana
                      ref={rivalRef}
    ...
    return (
      <div className="grid items-center border-b"
        ...
        {orderedDays.map((ymd) => {
          ...
          const commit = async () => {
            await setDayName(ymd, turn, local);
            await loadWeek(base);
          };
      ...
      {!hideHeader && (
        <header className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button onClick={discardAll} disabled={pendingCount === 0 || savingAll}
                    className="px-3 py-1.5 rounded-xl border hover:bg-gray-50 text-xs">Descartar</button>
          </div>
        </header>
      )}
