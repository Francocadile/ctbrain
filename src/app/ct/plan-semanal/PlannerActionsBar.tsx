...
export default function PlannerActionsBar({ onAfterChange }: { onAfterChange?: () => void; }) {
  ...
  async function doImport(file: File) {
    ...
    try {
      ...
      const payload: any = { sessions: Array.isArray(json.sessions) ? json.sessions : json };
  ...
  async function doDuplicate() {
    if (!dupFrom || !dupTo) return alert("Completá 'De' y 'A'");
    setDupBusy(true);
    try {
  ...
  async function csvValidateOrImport(file: File, apply: boolean) {
    ...
    try {
      const text = await file.text();
      const payload: any = { csvText: text, dryRun: !apply, overwrite: csvOverwrite };
    ...
    <>
      ...
      {showHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-[min(680px,92vw)] rounded-2xl bg-white shadow-xl">
            <div className="flex items-center justify-between px-4 py-3 border-b">
              <div className="font-semibold">¿Qué es “Herramientas”?</div>
              <button onClick={() => setShowHelp(false)} className="rounded-md border px-2 py-1 text-xs hover:bg-gray-50">Cerrar</button>
            </div>
            <div className="p-4 text-sm space-y-2">
              <p><b>Exportar</b> guarda la semana como JSON.</p>
              <p><b>Importar</b> carga un JSON exportado o una lista <code>sessions[]</code>. Podés alinear a otra semana.</p>
              <p><b>CSV</b> permite cargar celdas y banderas de día en masa, con validación previa (dry-run) y opción de limpiar lo existente.</p>
              <p><b>Duplicar</b> copia toda una semana a otra (incluye domingo gracias a fin exclusivo <code>lt: nextMonday</code>).</p>
              <p><b>Configuración de tabla</b> cambia <i>cómo se muestran</i> los nombres de filas sin alterar IDs internos (no rompe tus datos).</p>
            </div>
            <div className="px-4 py-3 border-t text-xs text-gray-500">
              Tip: Si algo sale mal al importar/duplicar, repetí la operación con “limpiar destino”.
            </div>
          </div>
        </div>
      )}
      ...
      <section className="rounded-xl border bg-white p-3 space-y-3">
        <div className="flex items-center justify-between">
          <div className="text-[12px] font-semibold uppercase">
            ...
            <HelpTip text="Exportar para respaldo/plantillas; Importar (JSON o CSV) para pegar contenido; Duplicar para copiar; Configuración para renombrar filas visibles." />
          </div>
          <button
            onClick={() => setShowHelp(true)}
            className="rounded-full border size-7 text-sm leading-7 text-center hover:bg-gray-50"
            title="¿Qué es esto?"
          >
            ?
          </button>
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
          ...
          <input type="date" className="rounded-md border px-2 py-1.5 text-sm" value={exportStart} onChange={(e) => setExportStart(e.target.value)} />
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="text-xs text-gray-500 ml-28">
            El archivo puede ser el JSON exportado o una lista <code>sessions[]</code>.
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2 ml-28">
            ...
            <button
              onClick={async () => {
                const f = fileRefCSV.current?.files?.[0];
                if (!f) return alert("Elegí un CSV");
                if (!csvPreview && !confirm("¿Aplicar sin validar? Recomendado validar primero.")) return;
          ...
          <div className="text-xs text-gray-500 ml-28">
            Columnas: <code>date, turn, row, title, place, time, video_label, video_url, day_flag, flag_rival, flag_logo</code>.
          </div>
        </div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="text-xs text-gray-500 ml-28">
            Copia todas las sesiones de la semana fuente a la semana destino.
          </div>
        </div>
