...
export default function CtLayoutClient({
  children,
}: {
  children: React.ReactNode;
}) {
  ...
  const isEmbeddedRoute = isEmbed || embedParam === "true";

  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row">
      {/* Header mobile (solo en sm, oculto en embed) */}
      {!isEmbeddedRoute && (
      ...
      {/* Fondo oscuro cuando el menú está abierto (solo mobile, no en embed) */}
      {menuOpen && !isEmbeddedRoute && (
        <div
      ...
      {!isEmbeddedRoute && (
        ...
        >
          ...
          <ul className="space-y-0.5 mb-2">
            ...
            <li>
              <NavItem href={"/ct/plantel" as Route} active={isActive("/ct/plantel")}>
                Plantel
              </NavItem>
            </li>
          </ul>
      ...
      <main
        className={
          isEmbeddedRoute
            ? "flex-1 w-full min-h-screen p-0 md:p-2"
            : "flex-1 p-3 md:p-4"
        }
      >
        {children}
